...
# Per-role deep comparison: OS, disks, services, IIS app pools
$primaryRoles = @(
    ...
    'SMS SQL Server',
    'SQL Server',
    'SMS Reporting Services Point'
)
$rolesForDetail = @($primaryRoles | Where-Object { $allRoles -contains $_ })
if (-not $rolesForDetail) { $rolesForDetail = @($allRoles | Select-Object -First 8) }

foreach ($role in $rolesForDetail) {
    $lSrv   = if ($lRoleToSrv.ContainsKey($role)) { $lRoleToSrv[$role] } else { $null }
    $rSrv   = if ($rRoleToSrv.ContainsKey($role)) { $rRoleToSrv[$role] } else { $null }
    ...
    $roleSb   = [System.Text.StringBuilder]::new()
    $roleDiffs = 0
    ...
    $inner, $dd = Build-KVTable "rsp_os_$role" $lOS $rOS @()
    $roleDiffs += $dd
    [void]$roleSb.Append("<h4 style='margin:10px 0 4px'>OS / Hardware $(Diff-Badge $dd "rsp_os_$role")</h4><div class='tbl-wrap'><table class='cmp'>$inner</table></div>")

    # Disk comparison (keyed on Drive letter)
    ...
    $inner, $dd = Build-ArrayTable "rsp_disk_$role" $lDisks $rDisks "Drive" @("FreeGB")
    $roleDiffs += $dd
    [void]$roleSb.Append("<h4 style='margin:10px 0 4px'>Disk Space $(Diff-Badge $dd "rsp_disk_$role")</h4><div class='tbl-wrap'><table class='cmp'>$inner</table></div>")

    # Services comparison (keyed on service Name)
    ...
    $inner, $dd = Build-ArrayTable "rsp_svc_$role" $lSvcs $rSvcs "Name" @()
    $roleDiffs += $dd
    [void]$roleSb.Append("<h4 style='margin:10px 0 4px'>Services $(Diff-Badge $dd "rsp_svc_$role")</h4><div class='tbl-wrap'><table class='cmp'>$inner</table></div>")
    ...
    if ($lIIS.Count -gt 0 -or $rIIS.Count -gt 0) {
        $inner, $dd = Build-ArrayTable "rsp_iis_$role" $lIIS $rIIS "Name" @()
        $roleDiffs += $dd
        [void]$roleSb.Append("<h4 style='margin:10px 0 4px'>IIS App Pools $(Diff-Badge $dd "rsp_iis_$role")</h4><div class='tbl-wrap'><table class='cmp'>$inner</table></div>")
    }
    ...
    if ($lWSUS -or $rWSUS) {
        $inner, $dd = Build-KVTable "rsp_wsus_$role" $lWSUS $rWSUS @()
        $roleDiffs += $dd
        [void]$roleSb.Append("<h4 style='margin:10px 0 4px'>WSUS Registry $(Diff-Badge $dd "rsp_wsus_$role")</h4><div class='tbl-wrap'><table class='cmp'>$inner</table></div>")
    }

    $safeRoleId = ($role -replace '[^a-zA-Z0-9]','_').ToLower()
    $rspCmpDiffs += $roleDiffs
    $title = "Role: $role — $lLabel vs $rLabel"
    [void]$rspCmpSb.Append("<div class='sub-section'><h3 class='sub-hdr'>$title $(Diff-Badge $roleDiffs "rsp_$safeRoleId")</h3>$($roleSb.ToString())</div>")
}
